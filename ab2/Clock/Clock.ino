...
void loop() {
    ...
    uint16_t ret;
    if (beep3) {
        M5.Speaker.tone(300, 50);              //Ton bei drücken von Button B
        beep3 = !beep3;
    }
    if (aon) {
        if (beep1) {
            M5.Speaker.tone(900, 100);                  //Ton bei gedrückthalten
            beep1 = !beep1;
        }
        ret = analogRead(36);
    ...
    if (execTime < millis()) {                        // 1s Verzögerung
        ...
        if (state) {                      // Digital oder Analog
            if (beep2) {
                M5.Speaker.tone(900, 100);              //Ton bei drücken von Button B
                beep2 = !beep2;
            }
        ...
        if (fon) {
            ...
            if (battery >= 75) {               //Batterie Level prüfen
